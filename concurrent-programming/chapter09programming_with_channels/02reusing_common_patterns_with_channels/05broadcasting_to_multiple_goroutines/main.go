...
func main() {
	...
	wordsMulti := Broadcast(quit, words, 2)                  // creares a goroutine that will broadcast the contents of the words channel to two output channels
	longestResults := longestsWords(quit, wordsMulti[0])     // creates the goroutine to find the longest words from the input channel
	...
	fmt.Println("Longest Words: ", <-longestResults)         // reads the result from the longestWords() goroutine and prints it
	fmt.Println("Most frequent Words : ", <-frequentResults) // reads the result from the mostFrequentWords() goroutine and prints it
}
