...
func main() {
	...
	for i := 0; i < 100; i++ {
		...
		if mutex.TryLock() {               // tries to acquire the mutex
			// if the mutex lock is available , it outputs frequency counts and releases the mutex
			for i, c := range AllLetters {
				fmt.Printf("%c-%d ", c, frequency[i])
			}
			mutex.Unlock()
		} else {
			// if the mutex is not available , it outputs a message and tries again later
