...
const (
	durationDecade = (time.Hour * 24 * 3650)
...
func generateKeyAndCertificate(template *x509.Certificate, duration time.Duration, keyOutputLocation string, certficateOutputLocation string, password string, parentKey *rsa.PrivateKey, parentCert *x509.Certificate) (*rsa.PrivateKey, *x509.Certificate) {
...
func generateX509Certificate(key *rsa.PrivateKey, template *x509.Certificate, duration time.Duration, parentKey *rsa.PrivateKey, parentCert *x509.Certificate) []byte {
	notBefore := time.Now()
...
func main() {
	rootKey, rootCert := generateKeyAndCertificate(&rootTemplate, durationDecade, "root_key.pem", "root_cert.pem", "password", nil, nil)
